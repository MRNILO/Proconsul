...
<h2>Cambia Clasificaciones</h2>

    <%Response.Write(ViewBag.dbesquemas)%>

    <br />
    <button class="btn btn-lg green" onclick="Agregar()">Agreger esquema +</button>
    <br />
    <br />
     <%Response.Write(ViewBag.TablaClasificaciones)%>
...
<!-- END PAGE LEVEL PLUGINS -->
    <script>
        ...
        function Elimina(idregistro) {
            if (confirm('¿Esta seguro que desea eliminar esta relación?')) {
                // Save it!

                $.ajax({
                    type: "POST",
                    url: '<%:Url.Action("Elimina_ComisionesClasificacion", "Administrativo")%>',
                    //dataType: "json",
  ...
  .fail(function () {
      alert("Error al procesar la solicitud.");
        ...
        function Agregar() {
            var idesquema = document.getElementById("esquema").value;
            var idclasificacion = document.getElementById("idclas").value;
                      
                $.ajax({
                    type: "POST",
                    url: '<%:Url.Action("Agregar_esquema", "Administrativo")%>',
                    //dataType: "json",
                    data: { id_clasificacion: idclasificacion, id_esquema: idesquema }
                })
  .done(function (msg) {
