...
<h2>Clasificaciones de comisiones <%:ViewBag.Mensaje%></h2>
      <a href="<%:ViewBag.Ruta_app%>Administrativo/NuevaRegla" class="btn btn-lg blue">Nueva Regla +</a>
    <br />
    <br />
    <br />
    <%Response.Write(ViewBag.TablaReglas)%>
</asp:Content>
...
<!-- END PAGE LEVEL PLUGINS -->
    <script>
        ...
        function Elimina(idregistro) {
            if (confirm('¿Esta seguro que desea eliminar esta regla?')) {
                // Save it!

                $.ajax({
                    type: "POST",
                    url: '<%:Url.Action("Elimina_Regla", "Administrativo")%>',
                    //dataType: "json",
                    data: { id_regla: idregistro }
                })
  .done(function (msg) {
